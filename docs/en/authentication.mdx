...
| `agent` | A [scrub node](/docs/scrubbing#managed-scrubbing-nodes)'s credential: the dataplane rule feed and its own report, and **nothing else**. |
...
every other endpoint — including the source-block channel, which installs kernel rules and so
stays operator-only — a compromised agent token, which lives on a remote scrub box, must not
