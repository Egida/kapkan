...
If you are not using [multi-tenancy](/docs/multi-tenancy) (a single deployment serving one
...
Five actions are audited. `ban`, `config_reload` and `source_block` are recorded on both
success and failure; `unban` and `source_unblock` are recorded only on success — a failed or
...
| `source_block` | `POST /api/v1/dataplane/sources` | `blocked` on success; `rejected` when policy refuses it. A refused block is itself an operator action, so it is attributed like a refused ban. |
...
| `action` | (all) | Filter to one of `ban`, `unban`, `config_reload`, `source_block`, `source_unblock`. |
...
| `action` | `ban`, `unban`, `config_reload`, `source_block`, or `source_unblock`. |
| `result` | `active`, `rejected`, `withdrawn`, `ok`, `error`, `blocked`, or `removed`. |
...
| `target` | The IP for `ban`/`unban`; `source->victim` for `source_block`/`source_unblock`; empty for `config_reload`. |
| `target_type` | `host` for `ban`/`unban`; `source` for `source_block`/`source_unblock`; `global` for `config_reload`. |
...
| `ban_state` | The final ban state for `ban`/`unban`; empty for `config_reload` and the source-block actions. |
