...
Nothing reports this for you. The kernel keeps a counter per rule, but no
operator-facing surface breaks static rules out individually yet, so a shadowed
rule looks exactly like a rule whose traffic never arrived. Order matters here
more than it does anywhere else in the config.
</Callout>
...
reported too. The verdict is also taken per address family, because the datapath is family-strict:
